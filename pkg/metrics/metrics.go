...
func (im *ControllerMetrics) computeL4ILBDualStackMetrics() map[L4DualStackServiceCount]int {
	im.Lock()
	defer im.Unlock()
	klog.V(4).Infof("Computing L4 DualStack ILB usage metrics from service state map: %#v", im.l4ILBDualStackServiceMap)
	counts := map[L4DualStackServiceCount]int{}
	...
	for key, state := range im.l4ILBDualStackServiceMap {
		...
		if state.Status == StatusError &&
			...
			time.Since(*state.FirstSyncErrorTime) > persistentErrorThreshold {
			state.Status = StatusPersistentError
		}
		counts[state.L4DualStackServiceCount]++
	}
	klog.V(4).Info("L4 ILB usage metrics computed.")
	return counts
}
...
func (im *ControllerMetrics) computeL4NetLBDualStackMetrics() map[L4DualStackServiceCount]int {
	im.Lock()
	defer im.Unlock()
